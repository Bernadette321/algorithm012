...
# 2. 迭代: 最常用版本(常用主要包括前序和层序,即[DFS和BFS]),[前中后]序遍历通用版本(一个栈的空间),
#         [前中后层]序通用版本(双倍栈(队列)的空间)
# 3. 莫里斯遍历: 利用线索二叉树的特性进行遍历
# 4. N叉树的前序遍历

# LeetCode题目: 144前序遍历, 94中序遍历, 145后序遍历, 102层序遍历, 589N叉树的前序遍历
...
'''
递归2: 通用模板,可以适用不同的题目,添加参数,增加返回条件,修改进入递归条件,自定义返回值
'''
class Solution:
    def preorderTraversal(self, root: TreeNode) -> List[int]:
        def dfs(cur):
